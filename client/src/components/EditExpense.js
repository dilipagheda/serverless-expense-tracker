...
const EditExpense = (props) => {
  ...
  const expenseItem = location.state.item
  ...
  const onSubmit = async (data) => {
    ...
    try
    {
      const createExpenseItemResponse = await axios.patch(ApiConfig.updateExpenseItemUrl(id), {
        title: data.title,
      ...
      if(data.expenseReceipt)
      {
        ...
        const generateUploadUrlResponse = await axios.post(ApiConfig.generateUploadUrl(id, data.expenseReceipt.name), null, {
          headers: {
            Authorization: idToken
          }
        })
  ...
  useEffect(() => {
    ...
    setExpenseDate(moment(expenseItem.date, 'DD/MM/YYYY').toDate())
    setValue("title", expenseItem.title)
    setValue("description", expenseItem.description)
    setValue("amount", expenseItem.amount)
    setValue("tags", expenseItem.tags.join(','))
  }, [register])
